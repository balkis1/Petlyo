...
const SECTION_ICONS = {
  'PERSONALITY': '🐾', 'FEEDING': '🍽️', 'DAILY ROUTINE': '🕐',
  'STRESS SIGNALS': '😟', 'COMFORT & SAFETY': '🏠',
  'MEDICATIONS': '💊', 'EMERGENCY': '🚨', "OWNER'S NOTE": '💛'
};

function openCareGuide() {
  const m = document.getElementById('modal-careguide');
  m.style.display = 'flex';
  m.classList.add('open');
  document.getElementById('cg-step-loading').style.display = 'none';
  document.getElementById('cg-step-result').style.display  = 'none';
  generateCareGuide();
}
...
async function generateCareGuide() {
  document.getElementById('cg-step-loading').style.display = 'block';
  document.getElementById('cg-step-result').style.display  = 'none';

  const petData = (typeof state !== 'undefined' && state.petData?.petName) ? state.petData : {
    petName: 'Luna', species: 'cat', traits: ['senior', 'anxious'], environment: ['calm/quiet', 'sitter home all day']
  };

  const speciesEmoji = { cat:'🐱', dog:'🐶', rabbit:'🐰', bird:'🦜', reptile:'🦎' };
  const emoji = speciesEmoji[(petData.species||'').toLowerCase()] || '🐾';

  try {
    const res = await fetch(`${BACKEND}/careguide`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    document.getElementById('cg-avatar').textContent = emoji;
    document.getElementById('cg-title').textContent  = `${petData.petName}'s Care Guide`;

    const sectionsEl = document.getElementById('cg-sections');
    ...
    function flushSection() {
      ...
      div.innerHTML = `<div class="cg-section-label">${icon} ${currentSection}</div><div class="cg-section-text">${currentText.join(' ').trim()}</div>`;
    ...
    lines.forEach(line => {
      ...
      const sectionMatch = Object.keys(SECTION_ICONS).find(k => trimmed.startsWith(k + ':'));
      if (sectionMatch) {
        flushSection();
        currentSection = sectionMatch;
        const rest = trimmed.slice(sectionMatch.length + 1).trim();
    ...
    document.getElementById('cg-step-loading').style.display = 'none';
    document.getElementById('cg-step-result').style.display  = 'block';

  } catch(e) {
    closeCareGuide();
    alert('Could not generate care guide: ' + e.message);
  }
}

document.getElementById('careguide-close')?.addEventListener('click', closeCareGuide);
document.getElementById('modal-careguide')?.addEventListener('click', e => {
  if (e.target === document.getElementById('modal-careguide')) closeCareGuide();
});
document.getElementById('btn-careguide')?.addEventListener('click', openCareGuide);
document.getElementById('cg-copy')?.addEventListener('click', () => {
  const sections = document.querySelectorAll('.cg-section');
  let text = document.getElementById('cg-title').textContent + '\n\n';
  sections.forEach(s => {
    const label = s.querySelector('.cg-section-label').textContent.trim();
    const body  = s.querySelector('.cg-section-text').textContent.trim();
    text += label + '\n' + body + '\n\n';
  });
  text += 'Generated by Petlyo · petlyo.vercel.app';
  navigator.clipboard.writeText(text).then(() => {
    const btn = document.getElementById('cg-copy');
    btn.textContent = '✓ Copied!';
